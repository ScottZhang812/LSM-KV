...
#define EXPECTI(exp, got,key) \
    expect<decltype(got)>((exp), (got), (key), __FILE__, __LINE__)
    template <typename T>
    void expect(const T &exp, const T &got,const long &key, const std::string &file, int line) {
        ++nr_tests;
        if (exp == got) {
            ++nr_passed_tests;
            return;
